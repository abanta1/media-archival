...
func SearchMovieMatch(title string, runtime int, apiKey string) (*TMDBResult, string) {
	...
	log.Log(7, "TMDB search URL: %s", searchURL)
	...
	log.Log(7, "TMDB search results: %d", len(searchResp.Results))
	...
	for _, movie := range searchResp.Results[:min(5, len(searchResp.Results))] {
		...
		log.Log(7, "Fetching details for '%s' (id=%d)", movie.Title, movie.ID)
		...
		log.Log(7, "  '%s' runtime=%d diff=%d", details.Title, details.Runtime, diff)
