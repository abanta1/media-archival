...
Required for **encoding workflows** (especially `v5`/`v6` encode pipelines). Used to transcode ripped titles with fine-grained control over codec, quality, audio tracks, and subtitles.
...
Required for **title matching and metadata lookup** in `v7-go`, `v5`, and `Get-DVDTitle.ps1`. A free API key is available by creating an account on The Movie Database.
...
- `v6/` – current PowerShell library modules used by the present workflow:
  - `Claude-Modules.ps1` – central bootstrap module that imports the rest.
...
- `v7-go/` – Go-based automated MakeMKV workflow.
